...
class ReferralIncentiveHistoryScreen extends StatefulWidget {
  const ReferralIncentiveHistoryScreen({
    super.key,
  });

  @override
  State<ReferralIncentiveHistoryScreen> createState() =>
      _ReferralIncentiveHistoryScreenState();
}

class _ReferralIncentiveHistoryScreenState
    extends State<ReferralIncentiveHistoryScreen> {
  bool isLoading = false;
  bool isFetching = false;
  final scroll_controller = ScrollController();

  final _referral_incentive_history_bloc = ReferralIncentiveHistoryBloc();
  late Stream<ResponseOb> _referral_incentive_history_stream;
  ...
  void initState() {
    ...
    _referral_incentive_history_stream =
        _referral_incentive_history_bloc.referralIncentiveHistoryStream();
    _referral_incentive_history_stream.listen((ResponseOb resp) {
      if (resp.success) {
        setState(() {
          for (var i = 0; i < resp.data.data.length; i++) {
            history_list.add([
              ...
              resp.data.data[i].seventyPercentAmount,
              resp.data.data[i].thirtyPercentAmount,
              resp.data.data[i].percent,
